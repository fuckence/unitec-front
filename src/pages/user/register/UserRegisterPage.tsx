...
import { Search, ChevronDown, X, Check, Loader2, ShieldCheck, AlertCircle } from "lucide-react";
    ...
    <div className="mb-5" ref={wrapperRef}>
      ...
      <div className="relative">
        ...
        <div className="absolute inset-y-0 right-0 flex items-center pr-3">
          {value ? (
            <button type="button" onClick={() => onChange("")} className="text-gray-400 hover:text-gray-500">
              <X size={18} />
            </button>
          ) : showSearchIcon ? (
        ...
        {open && !disabled && (
          <div className="absolute z-20 mt-2 w-full bg-white/95 backdrop-blur shadow-xl max-h-64 rounded-xl py-1 overflow-auto border border-gray-100">
            {filtered.length === 0 ? (
...
const UserRegisterPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate("/user/home", { replace: true, state: { registered: true } });
    } catch (error: any) {
    ...
    <div className="max-w-md mx-auto mt-24 px-4 pb-8">
      <div className="text-center mt-3 mb-6">
        ...
        <p className="text-sm text-red-500">
          {t("registration.hint", "Регистрация один раз")}
        </p>
      </div>
      ...
      {apiError && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-red-800">Ошибка регистрации</p>
      ...
      <div className="bg-white/90 backdrop-blur shadow-sm ring-1 ring-gray-100 rounded-2xl overflow-hidden">
        <form onSubmit={handleSubmit} className="p-5">
          ...
          <div className="mb-5">
            ...
            {errors.cabinet && <p className="mt-1 text-sm text-red-600">{errors.cabinet}</p>}
          </div>
